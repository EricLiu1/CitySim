                          ...
                          std::vector<glm::uvec3>& obj_faces) 
{
	...
	obj_vertices.clear();
	obj_faces.clear();
	bounds.push_back(glm::vec4(-1.0f, -1.0f, -0.5f, 1.0f));
	bounds.push_back(glm::vec4(1.0f, 1.0f, -0.5f, 1.0f));
/*
	for(int i = 0; i < 1; i++)
	{
                          ...
                          std::vector<glm::vec4>& bounds)
{
	...
	for(int i = 0; i < bounds.size(); i += 2) {
		...
		double z = lower_bound[2];

		double s = indices.size() * 3;

		obj_vertices.push_back(glm::vec4(xmin, ymin, z, 1.0f));
		obj_vertices.push_back(glm::vec4(xmin, ymax, z, 1.0f));
		obj_vertices.push_back(glm::vec4(xmax, ymin, z, 1.0f));
		indices.push_back(glm::uvec3(s++, s++, s++));

		obj_vertices.push_back(glm::vec4(xmax, ymax, z, 1.0f));
		obj_vertices.push_back(glm::vec4(xmax, ymin, z, 1.0f));
		obj_vertices.push_back(glm::vec4(xmin, ymax, z, 1.0f));
		indices.push_back(glm::uvec3(s++, s++, s++));
	}
